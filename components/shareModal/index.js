    ...
    >
      <div className={style.q}>
        <div className={style.name}>
          ...
          <span>
            {` regaló ${share.countCoffees} ${share.countCoffees > 1 ? "cafés" : "café"}`}
            {SHOW_DATE_COFFEE && ` el ${dayjs(share.createdAt).format("DD-MM-YYYY")}`}
          </span>
        </div>
        {share.message && <span className={style.text}>{share.message}</span>}
      </div>
      ...
      <div className={style.buttonShare}>
        <button className={style.buttonTwitter} onClick={() => shareTwitter()}>
