...
const Coffee = ({ setShare, coffee, loadNewCoffees, password, isAdmin }) => {
  ...
  const _deleteCoffee = async idCoffee => {
    const confirmDelete = window.confirm(`¿Estás seguro que querés borrar el mensaje?`);

    if (confirmDelete) await API.deleteCoffee({ idCoffee, password });
    loadNewCoffees();
  };
        ...
        <div className={style.q}>
          <div className={style.name}>
            ...
            <span>
              ...
              {SHOW_DATE_COFFEE && ` el ${dayjs(coffee.createdAt).format("DD-MM-YYYY")}`}
            </span>
          </div>
          {coffee.message && <span className={style.text}>{coffee.message}</span>}
        </div>

        {coffee.answer && <div className={style.answer}>{coffee.answer}</div>}
        ...
        {isAdmin &&
          (!isOpen ? (
            ...
            <>
              ...
              <button onClick={() => _sendAnswer(coffee._id)}>Enviar</button>
            </>
          ))}
      </div>
    </section>
